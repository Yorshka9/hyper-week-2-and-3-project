 ...
 function checkLetter(btn) {
  ...
  function resetGame() {
    if (reset === true) {
       ...
       for (let i = 0; i < lifeBar.length; i += 1) {
          lifeBar[i].src = "images/potte.jpg";
       }
   /*  for (let i = 0; i < letters.length; i += 1) {
  ...
  function checkWin() {
    if (letters.length === shownLetters.length) {
       ...
       overlay.style.backgroundImage = 'url(images/plants.jpg';
       reset = true;
  ...
  // when a letter button is selected, assign it with the class "chosen" and disable it.
   ...
   // if the checked letter is wrong, increment the value of 'missed' (++) and replace a liveHeart.png with a lostHeart.png 
            if (letterFound === false && missed < 3) {
              lifeBar[missed].setAttribute('src', 'images/lost-potte.jpg');
              missed++;
            }
         }
         checkWin();
   ...
   buttonReset.addEventListener("click", () => {
    ...
    body.style.backgroundImage = "url(images/plants.jpg)";
